...
- (NSString *)format:(NSString *)format {
    ...
    NSString *dataString = [dateFormateter stringFromDate:self];
    return dataString;
}
+ (NSString *)format:(NSString *)format {
    return [[NSDate date] format:format];
}
...
//        NSLog(@"--date:%@, year:%lu, =%lu", [lastdate dateAfterDay:-7 * i], [[lastdate dateAfterDay:-7 * i] year], year);
    }
...
- (NSString *)timeWeChatFormat {
    return [NSDate timeWeChatFormat:self];
}

+ (NSString *)timeWeChatFormat:(NSDate *)date {
    return [self timeInfoWithDateString:date];
}
 ...
 * 2016-12-11 本月之外
 */
- (NSString *)timeQQZoneFormat
{
    NSDate *curDate = [NSDate date];
    NSTimeInterval timeInter = [curDate timeIntervalSinceDate:self];
    
    NSString *timeStr;
    
    NSInteger day = [curDate day] - [self day];
    NSInteger month= [curDate month] - [self month];
    NSTimeInterval dayTime = 3600.0*24;
    
    if (day==0 && timeInter<dayTime) {
        timeStr = [NSString stringWithFormat:@"今天%@", [self format:@"HH:mm"]];
    } else if (day!=0 && timeInter<dayTime*2) {
        timeStr = [NSString stringWithFormat:@"昨天%@", [self format:@"HH:MM"]];
    }else if (month == 0 && timeInter<dayTime*[curDate day]) {
        timeStr = [self format:@"MM-DD HH:MM"];
    }else {
        timeStr = [self format:@"yyyy-MM-dd"];
    }
...
//992+时间+设备号+检验位 离线上传报案号
+ (NSString *)uploadPicName:(NSString *)deviceNum {
    
    NSString *name = [NSString stringWithFormat:@"992%@%@", [self format:@"YYYYMMddHHmm"], deviceNum];
    NSString *temp;
    int count = 0;
...
+ (NSString *)imageNameWith:(NSString *)deviceNum{
    
    return  [[self uploadPicName:deviceNum] stringByAppendingString:[NSString stringWithFormat:@"_%@",[self format:@"YYYYMMddHHmmss"]]];
